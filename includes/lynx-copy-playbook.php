...
> **🟢 LÄGE 2026-07-02 (ny session startar här):** **8 sidor omskrivna och LIVE på main** (PR #236 mergad, git-verifierat 2 jul): acne-ansikte, ytliga-blodkarl, behandla-pigmentflackar, hudforandringar, hudbehandlingar/ipl, om-oss, acne.php, ipl-rosacea. Alla facit-guidade (§13.D). **På staging väntar main-push:** d3ce9162 (hudforandringar död debug-rad — enda SIDÄNDRINGEN) + dagens playbook-/datacommits (2 jul: state-synk, optimeringar a–e, blindtest 3+4, 4 sidors facit, §1.5) — de senare är ofarliga för LYNX (`<?php exit`-filer), men main-pushen rensar debug-raden på produktion. **Kör 1 Claude i taget (§0.1) — parallellt övergivet.** **Modeller byggda:** AI COPY-rubrik v5+gradient (§1.3, ±1-band; blindtest 3+4 körda 2 jul: 57–62 % exakt på regressionsset, gradienten skärpt), sidnivå=MEDEL av block (§1.4, UPPGRADERAD från median 2 jul em: medel 9/9 på kända counts, median 12/15), AI QUESTIONS §1.5 (NY, hypotes), SCORE §1.2 (OMPRÖVAS — testprotokoll registrerat). **Process-optimeringar 2 jul:** §7.4b adversariell verifiering standard, arkiv-policy §11 (`lynx-log-arkiv.php`), facit-kö §9.1, Fas 2-attribueringsvakt §9. **Väntar på:** LYNX-refresh → kör mät-loopen (§10): mät FÖRE→EFTER per §11.1-bevakningslistan, validera modellerna (acne.php = A/B-test av SCORE/AI STYLE-spaken). **Nästa arbete (ägar-styrt 2 jul em):** statistiken 2 jul em SAMMANSTÄLLD (4 nya MODERATE-facit: solskadad-hy, rhinophyma, microdermabrasion, bristningar + helsajts-inventering + sidregister — allt i lynx-data/lynx-examples). **Microneedling-rewriten PÅBÖRJADES 2 jul em men ÅTERRULLADES på ägarbesked ("statistik först, den gör vi sen")** — block-plan bevarad i §11; starta INTE om den utan ägarens klartecken. **Facit-klara köer:** Fas 3b (§9: microneedling, portomning, acne-rygg, seborroisk-keratos, mogen-hy) + MODERATE-vågen (§8: rhinophyma, microdermabrasion, solskadad-hy, bristningar). **Facit SAKNAS för:** pigmentflackar (KW244) + oonskat-har (sista två MODERATE) samt Tier 1 (homepage, rosacea 221, perioral-dermatit 194). Per-block-facit + frågelistor finns REDAN sparade för kö-sidorna — be inte om dem igen; följ §7 inkl. 4b-verifiering (1 bedömare) och §8.1-gapbesluten; de 8 mätsidorna är FRYSTA tills LYNX mätt dem (§10 steg 5-backport därefter). **⚠️ Mätvarning 2 jul:** LYNX verkar ha ny version/skala — §1.2 OMPRÖVAS (SCORE 0 = trasig analys; 39-taket brutet utan våra ändringar); jämför färsk-mot-färsk, inte mot 30 jun-baselines (§11.1). §1.4 uppgraderad till MEDEL-modell 2 jul em (9/9; median ersatt på 12/15); modellöversyn på positiv-datan gjord 2 jul (§1.1 nyckel 5, §1.3-gradient, §1.4 additions-spak). **Data-princip:** spara allt ägaren skickar (lynx-data/examples), diffa vid ny scan, påminn om uppdateringar.
...
## 1.4 Sidnivå ≈ MEDEL av block-tiers (hur LYNX aggregerar block → sida) — uppgraderad från median 2 jul em
LYNX ger både **sidnivå-AI COPY** (Pages-listan) och **per-block-facit** (AI Copy-vyn). **Gällande modell (2 jul em): sidnivå ≈ VIKTAT MEDEL av alla betygsatta block** (🟢=0, 🔵=1, 🟠=2, 🔴=3; standardavrundning där exakt ,5 rundar uppåt/mot värre — 2,09 rundar alltså NED till 🟠, inte upp). **Medel: 9/9 träffar** på sidor med kända block-counts (micro 1,44→🔵 · porto 1,00→🔵 · acne-rygg 1,38→🔵 · sebo 1,25→🔵 · mogen-hy 1,57→🟠 · microdermabrasion 1,82→🟠 · bristningar 1,92→🟠 · rhinophyma 2,06→🟠 · solskadad-hy 2,09→🟠). **Median (historik, ersatt): 12/15** — missar mogen-hy (median 🔵, sida 🟠), solskadad-hy (median 🔴, sida 🟠) och rhinophyma (gränsfall→🔴, sida 🟠). De 6 äldre valideringssidorna (acne.php, acnebehandling, om-oss, ipl, behandla-pigment, hudforandringar) träffades av median och är ej medel-omräknade än (§9-TODO — kräver full block-count ur äldre facit). Full härledning i `lynx-examples.php` (§1.4-konsolideringen). **Starkaste modellen vi har.**
...
**Strategisk implikation (styr prio + förväntan) — medel-logik ger RÄKNEBAR väg till badge-byte:**
- **Sänk SIDNIVÅN = sänk MEDLET under gränsen** (🔵-badge kräver medel <1,5; 🟠 <2,5). Varje 🔴→🔵-fix sänker summan med 2 poäng → **räkna per sida hur många block-fixar som krävs**: mogen-hy 1,57 (n=14) = 1 fixat 🔴 räcker (→1,43) · microdermabrasion 1,82 (n=11) = 2 · bristningar 1,92 (n=13) = 3 · rhinophyma 2,06 (n=16) = 5 · solskadad-hy 2,09 (n=11) = 3–4.
...
## 1.5 AI QUESTIONS / AI Overview-modellen (NY 2026-07-02 — hypotes byggd på 5 sidors data)
LYNX-vyn **"AI Overview Ranking"** per sida listar frågorna sidan borde äga i AI-overviews: **SCORE** = frågans vikt (högst först), **OPTIMIZED** = hur väl sidan besvarar frågan idag (**GOOD** > OK > **POOR**; "–" = ingen status). Sidnivå-kolumnen AIQ (GOOD/OK/POOR) aggregerar troligen denna vy. Rådata per sida i `lynx-data.php` (2 jul: micro/porto/acne-rygg/sebo + mogen-hy 27 frågor).
...
**MODERATE-vågen:** ✅ KLARA: `om-oss.php`, `hudbehandlingar/ipl/`, `ipl-rosacea.php` (KW20, P.PRICE HIGHEST, SCORE 39 — facit-driven + workflow-verifierad). **KVAR (7):** `pigmentflackar.php` (KW244, **facit SAKNAS**), `hudproblem/oonskat-har/` (**facit SAKNAS**), `rhinophyma-rosacea.php` **(FACIT ✅ 2 jul em: 8🔴/2🟠/5🔵/1🟢 + 2 live-grammatikfel)**, `microdermabrasion.php` **(FACIT ✅ 2 jul em: 5🔴/1🟠/3🔵/2🟢)**, `mogen-hy.php` **(FACIT ✅ KOMPLETT 2 jul: 5🔴/1🟠/5🔵/3🟢)**, `solskadad-hy.php` **(FACIT ✅ 2 jul em: 6🔴/1🟠/3🔵/1🟢 + 1 live-grammatikfel)**, `hudproblem/bristningar/` **(FACIT ✅ 2 jul em: 6🔴/1🟠/5🔵/1🟢)**. Alla facit i `lynx-examples.php`. (Prioordning: §9.1.)
...
- [ ] **GAPS-rensningen 2 jul:** ytliga-blodkarl 2/2→0/0, ipl-rosacea 2/1→0/0, homepage 3/2→0/0 — utan att sidorna refreshats (tiers/SCORE står stilla). Verifiera vid nästa scan: äkta rensning (gap-texterna lösta/borttagna av LYNX) eller scan-artefakt? Påverkar §8.1-gapbesluten för de sidorna.
...
- [ ] **§1.4 medel-omräkning (kvarstår för de 6 äldre):** medel UPPGRADERAD till primär modell 2 jul em (9/9 på kända counts; median 12/15 — se §1.4). Kvar: räkna om acne.php, acnebehandling, om-oss, ipl, behandla-pigment, hudforandringar med viktat medel (kräver full block-count ur äldre facit i lynx-examples).
- [ ] **Homepage-raden i AICOPY-vyn:** homepage `/` syntes INTE i helsajts-dumparna 2 jul em (fanns 30 jun: KW296, AIQ POOR) — be ägaren verifiera var den ligger, och vad rad 15 faktiskt är (läst som "/behandlingar.php" SLIGHT/POOR — filen finns inte i repot; kan vara felläst homepage-rad).
...
2. `pigmentflackar.php` — KW244, 🟠 MODERATE. Största MODERATE; pigment-huvudsida (silo mot behandla-pigmentflackar).
