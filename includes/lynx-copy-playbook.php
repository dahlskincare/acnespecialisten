...
> **🟢 LÄGE 2026-07-02 (ny session startar här):** **8 sidor omskrivna och LIVE på main** (PR #236 mergad, git-verifierat 2 jul): acne-ansikte, ytliga-blodkarl, behandla-pigmentflackar, hudforandringar, hudbehandlingar/ipl, om-oss, acne.php, ipl-rosacea. Alla facit-guidade (§13.D). **På staging väntar main-push:** d3ce9162 (hudforandringar död debug-rad — enda SIDÄNDRINGEN) + dagens playbook-/datacommits (2 jul: state-synk, optimeringar a–e, blindtest 3+4, 4 sidors facit, §1.5) — de senare är ofarliga för LYNX (`<?php exit`-filer), men main-pushen rensar debug-raden på produktion. **Kör 1 Claude i taget (§0.1) — parallellt övergivet.** **Modeller byggda:** AI COPY-rubrik v5+gradient (§1.3, ±1-band; blindtest 3+4 körda 2 jul: 57–62 % exakt på regressionsset, gradienten skärpt), sidnivå=MEDIAN av block (§1.4, 10/11 — första missen mogen-hy 2 jul em, medel-hypotes registrerad), AI QUESTIONS §1.5 (NY, hypotes), SCORE §1.2 (OMPRÖVAS — testprotokoll registrerat). **Process-optimeringar 2 jul:** §7.4b adversariell verifiering standard, arkiv-policy §11 (`lynx-log-arkiv.php`), facit-kö §9.1, Fas 2-attribueringsvakt §9. **Väntar på:** LYNX-refresh → kör mät-loopen (§10): mät FÖRE→EFTER per §11.1-bevakningslistan, validera modellerna (acne.php = A/B-test av SCORE/AI STYLE-spaken). **Nästa arbete:** **Fas 3b-kön (§9) = 5 facit-klara sidor** (microneedling, portomning, acne-rygg, seborroisk-keratos + mogen-hy tillagd 2 jul em, facit komplett + AI Overview-lista 27 frågor sparad; rewrite i EGEN session, ägarbesked 2 jul; fler sidor aviserade); därefter Tier 1 (§9.1: Homepage KW296, pigmentflackar 244, rosacea 221, perioral-dermatit 194 — facit först). **Ny session startar med "kör Fas 3b" → börja `microneedling.php`; per-block-facit + AI Overview-frågelistor finns REDAN sparade (lynx-examples + lynx-data) — be inte om dem igen; följ §7 inkl. 4b-verifiering (1 bedömare) och §8.1-gapbesluten; de 8 mätsidorna är FRYSTA tills LYNX mätt dem (§10 steg 5-backport därefter).** **⚠️ Mätvarning 2 jul:** LYNX verkar ha ny version/skala — §1.2 OMPRÖVAS (SCORE 0 = trasig analys; 39-taket brutet utan våra ändringar); jämför färsk-mot-färsk, inte mot 30 jun-baselines (§11.1). §1.4 median-modellen nu 10/11 (mogen-hy-missen 2 jul em → medel-hypotes, §1.4); modellöversyn på positiv-datan gjord 2 jul (§1.1 nyckel 5, §1.3-gradient, §1.4 additions-spak). **Data-princip:** spara allt ägaren skickar (lynx-data/examples), diffa vid ny scan, påminn om uppdateringar.
...
LYNX ger både **sidnivå-AI COPY** (Pages-listan) och **per-block-facit** (AI Copy-vyn). Reverse-engineerat 2026-07-01: **sidnivån ≈ MEDIAN-tiern av alla renderade block** (mittblocket sorterat 🟢→🟥; räkna även responsiva dubbletter; gränsfall rundar mot värre tier). Validerat **10/11** (acne.php 🔵 SLIGHT; acnebehandling 🔵; om-oss 🟠 MODERATE; ipl 🟠; behandla-pigment 🔴 SIGNIFICANT; hudforandringar 🔴; + 2 jul: microneedling 🔵, portomning 🔵, acne-rygg 🔵, seborroisk-keratos 🔵 — alla ✓; **FÖRSTA MISSEN: mogen-hy 2 jul em** — komplett facit ger median 🔵 men sidnivå 🟠). **NY MEDEL-HYPOTES (2 jul em):** viktat medel (🟢=0 🔵=1 🟠=2 🔴=3, avrunda mot värre) träffar mogen-hy (1,57→🟠 ✓) OCH alla 4 nya facit-sidor — de 6 äldre ej omräknade än (§9-TODO); passar alla 11 → uppgradera median→medel. Full härledning i `lynx-examples.php`. **Starkaste modellen vi har.**
...
- **En 🔵-sida sänks INTE av att fixa enstaka 🔴-block** (medianen redan bra), MEN det höjer SCORE (AI STYLE-komponenten — §1.2 OMPRÖVAS) + blockkvaliteten → gör det för SCORE, inte AI COPY-flaggan.
...
**MODERATE-vågen:** ✅ KLARA: `om-oss.php`, `hudbehandlingar/ipl/`, `ipl-rosacea.php` (KW20, P.PRICE HIGHEST, SCORE 39 — facit-driven + workflow-verifierad). **KVAR (7):** `pigmentflackar.php` (KW244), `rhinophyma-rosacea.php`, `microdermabrasion.php`, `mogen-hy.php` **(FACIT ✅ 2 jul — ⚠️ möjligen trunkerat, be om resten; se lynx-examples)**, `solskadad-hy.php`, `hudproblem/bristningar/`, `hudproblem/oonskat-har/`. (Prioordning: §9.1.)
...
- [ ] **§1.4 medel-omräkning:** räkna om de 6 äldre valideringssidorna (acne.php, acnebehandling, om-oss, ipl, behandla-pigment, hudforandringar) med viktat medel (🟢0/🔵1/🟠2/🔴3, avrunda mot värre) — median missade mogen-hy (10/11), medel träffar alla 5 sidor med kända counts. Passar alla 11 → uppgradera §1.4 median→medel.
