...
**Fem nycklar:**
1. **Öppningen sätter GOLVET** — mekanism/konkret vs fyllnad/marknad. *Tolka via nyckel 5:* en enstaka mild fras är ingen fyllnadsöppning om konkretionen kommer direkt. **MEN fluff-ackumulering × längd höjer graden:** ett kort praktiskt block klarar 🔵 även med ett marknadsord (behandla-pigment banner/about-1); ett långt block med staplad marknad/hedge blir 🔴/🟥 trots mekanism (behandlingskorten). → Byt fyllnadsöppning mot mekanism/konkret **och** kapa ackumuleringen. *(Samma behandling, olika betyg: laser-kortet 🔵 på acnebehandling, 🔴 på behandla-pigment — `lynx-examples`.)*
2. **🔵-ribban är hög** — HELA blocket måste vara tätt. Halvkonkret = 🟠.
3. **Kort + konkret siffra/mått/namngiven maskin → 🟢** ("större än 5 mm", "1000 kr", "Dermapen 4"). Längd är modifierare, inte spak.
4. **Fluff-svansen fäller ett annars konkret block** ("perfekt anpassad för dina unika behov", "säkerställa en trygg upplevelse") — **och det gäller även en cirkulär summeringssvans som bara upprepar blocket, även utan ordet "Sammanfattningsvis"** (microneedling på acnebehandling: full mekanism men 🔴 pga längd + svans). Korta konkreta instruktioner ("håll rent, avstå smink första dygnet") klarar 🔵 även med ett milt "det är viktigt att".
5. **Ackumuleringen fäller, inte den enstaka frasen** *(2 juli, första SLIGHT-tunga facitet: 23🔵/9🟢 av 53 block)*. Fyllnadsfras + **konkretion direkt i mening 1–2** = 🔵 (acne-rygg "flera olika faktorer" → talg-mekanism i mening 2); fyllnadsfras + mer fyllnad = 🔴 (demografi-blocken). **Konkreta process-/stegblock tål flera marknadsord** (portomning: nästan allt 🔵 trots effektiv/skonsam/idealisk — Vapozon/lansett/peelingtvätt bär). **CTA-pivot-räddningen gäller även behandlingskort** (belagt 3 sidor). **FAQ-svar kan nå 🟢** → Fas 2-FAQ är även en AI COPY-/medel-spak (§1.4). **🔵↔🟢 är brus utan operativ betydelse** — båda = lämna (§13.D); lägg ingen prediktionsmöda där.
...
Gissa en blocks AI COPY-tier (🟥 EXTREME > 🔴 SIGNIFICANT > 🟠 MODERATE > 🔵 SLIGHT > 🟢 HUMAN). **Fem ofuskade blindtest; full data i `lynx-examples.php`. Operativa slutsatser:**
1. **Exakt tier är BRUS** — LYNX:s mid-tier-gränser är luddiga och sid-kontextberoende ("om oss"-prosa mildare än behandlingsprosa). **Det pålitliga är ±1-bandet.** Använd modellen som ett band ("troligen SLIGHT–MODERATE"), aldrig som exakt etikett.
2. **v5 presterar** 57–62 % exakt / 89–92 % ±1 på regressionsseten; **äkta out-of-sample 55 % / 90 % ±1** (blindtest 5) → modellen generaliserar; semi-holdout-farhågan (38 %) motbevisad.
3. **Systematisk miss: marknad/passiv på BEHANDLINGSBLOCK undergissas ett steg.** Facit är hårdare mot marknad än intuitionen. **Luta åt 🔴 vid tvekan på behandlingsprosa.**
4. **v5 = §1.3 synkad med §1.1.** Vi mikro-tunar inte per sida (v2↔v3 pendlade = overfit) — ändra bara när ett mönster upprepas över **≥2 sidor**.
5. **Facit trumfar alltid modellen.** Prediktionen styr prio och förväntan, aldrig beslutet om vad som ska skrivas om.
...
LYNX ger både **sidnivå-AI COPY** (Pages-listan) och **per-block-facit** (AI Copy-vyn). **Arbetsmodell: sidnivå ≈ VIKTAT MEDEL av alla betygsatta block** (🟢=0, 🔵=1, 🟠=2, 🔴=3, 🟥=4; standardavrundning, exakt ,5 uppåt — 2,09 rundar NED till 🟠).

- **Ställning (2 juli em):** medel **14/17** = median **14/17**, på **disjunkta missmängder**. Medel är perfekt på de 11 färska kompletta faciten (micro 1,44🔵 · porto 1,00🔵 · acne-rygg 1,38🔵 · sebo 1,25🔵 · mogen-hy 1,57🟠 · microderm 1,82🟠 · bristningar 1,92🟠 · rhinophyma 2,06🟠 · solskadad 2,09🟠 · pigmentflackar 2,06🟠 · oonskat-har 1,77🟠) men missar 3 av 6 äldre; median har spegelvänd profil.
- **Killer-observationen:** hudforandringar 2,08 → 🔴 men solskadad 2,09 → 🟠 ⇒ **badgen är INTE en ren funktion av tier-mixen.** Alternativa aggregeringar testade och fällda (`lynx-examples`).
- **Strukturell förklaring (officiell tooltip):** badgen är *"Estimated level of AI-generated content on this page"* — en **helsides-skattning av en kontinuerlig nivå**, inte ett aggregat av block-betyg. Medel/median är PROXYS; två sidor med nästan samma tier-mix kan legitimt hamna olika (textmängds-viktning, WORDS). **Gråzonen är strukturell och går inte att räkna bort.**
- **Regler:** (1) medel <1,5 → förvänta 🔵. (2) **1,9–2,4 = GRÅZON** — prediktera inte hårt där. (3) block-fix-räkningar = ESTIMAT. (4) modellen styr förväntan/prio; arbetsmängden styrs av §7 (fixa alla 🔴/🟠) oavsett. (5) räkna betygsatta FAQ-rader i n när de syns.
- **Första EFTER-datapunkten (6 juli, same-scan badge + blocklista):** acne-ansikte omskriven → block-medel 0,89 → pred 🔵, badge SLIGHT ✓ → medel 15/18 (preliminärt: FAQ-block ev. utanför dumpen).
- **ÖPPNA BESTÄLLNINGAR (§9, ordagrant):** färsk analys där **sidnivå-badge + KOMPLETT per-block-lista (inkl. FAQ-svar) kommer ur SAMMA scan**. · **om-oss** (EFTER-medel 0,91 → pred 🔵) räknas IN i statistiken först när **badge-raden bekräftats**.
...
### 4.2 Kund-tilltal & extern best practice (importerad 2026-07-06 — Google people-first, Konsumentverket, klarspråk; käll-URL:er i §5-juridikblocket nedan; postens fulltext i `lynx-log-archive` 6 juli #5)
...
1. **Spara LYNX-baseline** för sidan i §11 (AI COPY, SCORE, GAPS, AI QUESTIONS, PAGE TYPE, P. PRICE, KW, **CLICKS + WORDS — krävs för SEO-vakten §10.0**, ev. RANK) — innan något ändras. **Notera ANALYSDATUM PER VY** (AI Copy-vyns "Last update" + SCORE-popupens "Analyzed X ago") — vyerna refreshar oberoende (§11.1-tillägget 6 jul), så radens kolumner kan ha olika ålder. *(Obs: SCORE 0/blank = pågående omanalys/indexering som sätter sig själv — ägarbesked 7 jul; logga inte 0 som baseline, notera "väntar analys" och läs av nästa dump i stället.)*
...
**B. Filerna är INTE webb-läsbara (löst).** Alla `includes/lynx/`-filer börjar med `<?php exit; ?>` → servern kör dem och returnerar **tomt** om någon öppnar URL:en. Vi läser dem i editor/git. Inget behov av deploy-exkludering eller radering. (Verifiera vid behov: öppna någon av dem på temp-staging → ska vara tom.)
...
**D. Läs ALLA blocktyper — REWRITE är FACIT-DRIVEN, men lämna inte resten oöverskådad.**
- **Skriv om 🔴/🟠/🟥** (prioritet). **Se även över 🔵/🟢 med lätt hand** — bara tydliga fixar (typo, grammatik, tappat sökord, uppenbar tell), **aldrig stilistisk över-omskrivning av OK-block** (v3-felet: "rör inte det som är bra").
- **Utan per-block-facit:** utgå från hela sidan + §1.3-modellen; lämna hellre vid SLIGHT↔MODERATE-tvivel.
- **Bedöm ALLTID sidan som HELHET** — flöde mellan block, dubbletter över block, sökordsspridning, att ett 🔵-block inte skaver i sitt sammanhang. Facit prioriterar; helheten avgör.
- **Obligatoriska blocktyper per sida:** (1) banner-description, (2) ProblemTrivia `content`, (3) ProblemTrivia `extended_content` ("Läs mer"), (4) service-/behandlingskort, (5) FAQ-svar. "Klar" = alla klarar §3.
...
**O. Minimal diff = steg för steg, ta INTE bort substans (default-läge för all omskrivning).**
**Rättesnöret:** längden får bli kortare *eller* längre — skadan är att **ta bort för mycket på en gång** (hela stycken, listor, sakpoänger). Behåll varje block, varje list-punkt, varje poäng. En ren summering eller dubblett får trimmas, men som ETT medvetet litet steg ägaren ser, aldrig som del av ett svep.
**Jobba steg för steg:** några få ändringar → visa ägaren → iterera. Ett helt omskrivet sidsvep går varken att granska eller attribuera. Så tappade acne-ansikte-listan (fångad före commit) och ytliga-blodkarl (rullad tillbaka: "för stora förändringar") substans.
**Säkrare att ta bort för lite än för mycket** — vi kan alltid kapa mer när mät-loopen (§10) visar att det krävs; bortskuren ton eller poäng är svår att få tillbaka. Behåll struktur, listor, avslutande stycken och det mesta av ordalydelsen. Justeringar får lika gärna **lägga till** som ta bort. Rätt info på rätt ställe — utgå från helheten (§13.D).
*(Princip, inte mikroregel — vi rutar inte in oss, §0/§5. Största spaken med minst risk står i §1.1 nyckel 1 och upprepas inte här.)*
