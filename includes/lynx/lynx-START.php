...
> **🟢 LÄGE 2026-07-07** *(ny session: läs denna banner → routern överst → rätt fil för din uppgift; backlog §8/§9 nedan)*
...
- [ ] **1. Ägarbeslut: godkänn/backa lynx-data-omstruktureringen** (commit c5c34e0e: AKTUELLT LÄGE-tabell + baseline-register i lynx-data.php, snapshots verbatim → nya lynx-data-arkiv.php, 519→183 rader; backas med `git revert`).
- [ ] **2. Stale state + trasiga pekare:** §9 Fas 3-raderna säger "väntar main-push/⛔GSC-gate" på 6 sidor fast ALLT är LIVE på main sedan 6 juli (logg #13) — även §8 (d3ce9162) + §9.1 Tier 3; TOC-raden ("Innehåll:") listar §§ som flyttat till rewrite/models/logg vid splitten; routerns "(save-first, §0)"-pekare; lynx-log-arkiv + logg-intron säger "playbooken"; lynx-examples rad 4 pekar på stubben.
- [ ] **3. Flytta 3 unika saker ur loggen** (förkrav för steg 4): milier-/CryoPen-prisverifieringen (öppen ägar-tråd, post 2 juli em #4) → §9 Bevaka · juridik-käll-URL:erna (post 6 juli #5) → lynx-rewrite §5 · state-synk-principen ("greppa gamla frasen, synka ALLA förekomster") → §0.
- [ ] **4. Loggpost-MALL i §11-intron + arkivera ~20 stängda poster** (2 juli-sjoket + 6 juli #3/#4/#5/#6/#9/#10/#12 + 1 juli main-push — alla ratificerade/ersatta per arkivpolicyn; fulltext → lynx-log-arkiv, enradare kvar) + komprimera #14. Sid-poster med omätta baselines RÖRS EJ. Effekt ≈ −40 %.
- [ ] **5. lynx-models:** arkivera §1.2:s superseded historiklager (OMPRÖVAS-noteringen, 30 juni-multiplikativa analysen, besvarade testprotokollet) → pekare; konsolidera §11.1:s FEM staplade ⚠️-mätregelstycken till ETT 5-punktsblock; spak-kartan: EEAT = ej copy-spak, AI STYLE-intervall färska skalan; MÄTT-radernas statusceller → kort + logg-pekare. Evidence-§5-filtret + no-regression + §1.5 + §10.0 RÖRS EJ.
- [ ] **6. lynx-score:** slutsatserna A–F → kompakt slutläges-tabell (utan korrigeringshistorik); arkivera gamla-skalans sektion + 2 juli-sektionen (sammanfattningsrader kvar: formeln 7/7, datapunkterna, acne.php no-regression-subs); fixa models rad 3-pekaren (beslut: härledningen BOR i lynx-score; models §1.2 = operativa sammanfattningen) + models stale "klampas på hälsosidor"-formulering.
- [ ] **7. lynx-START-omstrukturering** (störst läsvinst): LÄGE ≤8 rader; facit-flödesväggen (rad 24) → 4–5 rader + pekare till spara-receptet i lynx-data; §8/§9 KLAR-rader → enradare med logg-pekare (öppna rester bevaras: oonskat-har-ägarfrågan, micro-AIQ-detaljen, pigment V2-prediktion + AI STYLE 55); lösta [x]-bevakningstrådar arkiveras; Bevaka-trådarna → beslutspunkt + öppen fråga (historik → logg); §12 flyttas upp under routern; avduplicera prime-direktiv/filkarta (står 3–5 ggr).
- [ ] **8. lynx-rewrite — ENDAST formatkomprimering** (inga regeländringar): §1.1-väggen → 5 punktrader; §1.3-ingressens blindtest-saga → 5 operativa slutsatser + pekare till examples; §1.4 → ~8 rader (öppna beställningar oavkortade); §13.O:s tre stycken → ett (spak-regeln EN gång); §13.D → punktlista. §2–§7, §4-banken, §5-juridiken, §3-checklistan RÖRS EJ.
- [ ] **9. lynx-examples + småfiler:** blindtest 1/3/4 + MODELLANALYS 2 juli + median-hypotesen + FÖRE-tabellerna för de 3 MÄTTA sidorna (FÖRE-tiers finns i EFTER-tabellernas kolumn) → ny lynx-examples-arkiv.php; ALLA facit för omätta/kösidor kvar. Stubben lynx-copy-playbook 23→~6 rader; SERP-drift-regeln (questions rad 13) → models §1.5; gaps/questions-analysparenteser → pekare (rålistorna orörda).
- [ ] **10. Slutverifiering:** no-loss-koll mot underlags-filens MÅSTE-BEHÅLLAS-listor, grep-koll av pekare, loggpost enligt nya mallen, radera lynx-optimering-underlag.php, push.
