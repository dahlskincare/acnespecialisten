...
**Sessionens allra första rad är hälsningen — före ALLA verktygsanrop, även `date` och läsningen av läges-filerna.** Neutral och kort: "Hej!" + vad som händer härnäst — att du kontrollerar att ingen annan är igång, läser läget och **sedan presenterar alternativen**. Två meningar räcker. Verktygsanrop får följa i samma meddelande, men ingenting får stå före raden. **Tidshälsningen ("God morgon" / "God eftermiddag" / "God kväll") öppnar i stället ÖPPNINGSLISTAN** — kolla klockan med `date` under ritualen, gissa inte; `date` får numera buntas med steg 1-kommandona. *(Omskriven 10 juli efter TREDJE intro-incidenten, logg #56: sessionen skrev hälsningen exakt enligt dåvarande regel — efter `date`, före `git status` — och ägaren såg den ändå aldrig. Text mitt i en verktygskedja garanteras inte visas i terminalen; pålitligt visas bara turens FÖRSTA meddelande och det SISTA. Därför bor hej:et i det första och tidshälsningen i det sista. De två tidigare skärpningarna (hoppad hälsning · `date`+`git status` i samma svep, logg #55) botade beteendet men inte visningen.)*
