...
> - **🔧 PÅGÅENDE FÖRST: fil-optimeringspasset §9.0** — steg 1–4 klara + **mål-arkitekturen RATIFICERAD 8 juli** (lag: "ett fakta = en cell, celler pekar"; START blir tunn dispatcher, ny `lynx-backlog.php`, loggen = HISTORIK). **NÄSTA: steg 5 (lynx-models).** Arbetssätt: ETT steg i taget med ägar-avstämning, INGA agent-utskick; spec i `lynx-optimering-underlag.php`.
...
- [ ] **9. lynx-examples-splitten + småfiler** (632 rader = dolda jätten): blindtest 1/3/4 + MODELLANALYS 2 juli + median-hypotesen + FÖRE-tabellerna för de 3 MÄTTA sidorna (FÖRE-tiers finns i EFTER-tabellernas kolumn) → ny `lynx-examples-arkiv.php` (HISTORIK); ALLA facit för omätta/kösidor kvar i `examples-aktiv`. **+ Ägarbeslut 8 juli: microneedlings block-plan flyttas hit ur logg em #2**, in under sidans facit-sektion (loggen blir därmed ren HISTORIK — inget operativt beror på den). **Stubben `lynx-copy-playbook.php` RADERAS** (ej kortas — den är en redirect till en fil som alla pekare nu träffar direkt); grep-koll att inget pekar på den. SERP-drift-regeln (questions rad 13) → models §1.5; gaps/questions-analysparenteser → pekare (rålistorna orörda).
- [ ] **10. Slutverifiering + permanentning:** **skriv struktur-kartan (triad × domän + laddnings-recept + "ett fakta = en cell"-lagen) i START §0** — annars dör den med underlagsfilen. Front-matter-manifest på ALLA filer. No-loss-koll mot underlags-filens MÅSTE-BEHÅLLAS-listor; grep-koll att varje §-pekare träffar rätt fil (särskilt §8/§9/§12 → backlog, och de raderade stubbarna); loggpost enligt nya mallen; radera `lynx-optimering-underlag.php`; push.
...
| Sida | Status | Tid |
|------|--------|-----|
| *FIL-OPTIMERINGSPASSET §9.0 — steg 1–4 klara, mål-arkitekturen ratificerad. NÄSTA: steg 5 (lynx-models). Spec i `lynx-optimering-underlag.php`. Sidarbete pausat under passet; därefter microneedling (Fas 3b).* | pågående | 8 juli |
