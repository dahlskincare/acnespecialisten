...
- [ ] **[medel] PHP-fel renderas publikt på både live och staging (display_errors PÅ)** — live svarar i dag med parse error + serverns hemkatalog i klartext på `/includes/lynx/lynx-START.php` (main:s gamla version, rad 192; upptäckt 10 juli vid main-push-undersökningen). Stagings START är ren och lagar just den vid nästa merge men felklassen kvarstår för varje framtida fel (samma mekanism som warning-läckan på hudforandringar, logg #13). **Förslag:** be webbhotellet stänga av display_errors i produktion — fel ska till serverloggen, inte till besökaren.
- [ ] **[medel] php-öppningstaggen i löptext är en parse-fälla i lynx-filerna** — exit-vakten på rad 1 stoppar exekvering men INTE parsning: citerar brödtexten taggen bokstavligt försöker PHP tolka resten. `lynx-log-archive.php` rad 200 fäller hela filen (publikt parse error på staging nu och följer med till live vid merge); arkivets rad 372 + mall-review-raden i §9 ovan + en rad i `lynx-rewrite` parsar av ren tur och är en redigering från att smälla. **Förslag:** (a) neutralisera taggen på arkivrad 200 före main-push (HISTORIK = ägarbeslut), (b) regelrad i START: skriv aldrig taggen bokstavligt i brödtext, skriv "php-öppningstaggen".
