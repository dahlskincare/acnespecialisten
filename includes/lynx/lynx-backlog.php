...
| Sida | Status | Tid |
|------|--------|-----|
| *(tom — ingen är igång)* Senast: boilerplate-sweepen 27 filer ✅ 10 juli (logg #46). **Kön i §9 är öppen — ägaren väljer.** | – | – |
...
- [ ] `microneedling.php` (färsk FÖRE 8 juli: SCORE **60**, KW 63, 🔵; facit 22 apr: 4🔴/2🟠) — de-AI + "microneedling kur"-gapet (§8.1). **⚠️ ÅTERRULLAD TVÅ GÅNGER: 2 juli (ägarbeslut, statistiken först) och 8 juli (§7.4b-granskningen underkände — §5-brott, tappad substans, påhittade påståenden).** Läs **hela REWRITE-SPEC:en i `lynx-examples`** innan du börjar: block-plan + sex verifierade fällor + sju gränsfall. 5 språkfel/typos redan fixade 2 juli em.
  **⛔ FÖRVILLKOR — be ägaren om hela SCORE-popupen INNAN ett tecken ändras (öppnat 8 juli).** Vi har bara Overall 60; ingen nedbrytning. **Fångar vi alla 12 delvärden före main-push blir fingeravtrycket självt färskhetsdetektorn efteråt:** rör sig Evidence eller Language är crawlen bevisligen ny, och då är AI STYLE-värdet giltigt som EFTER **även om det står stilla**. Det är enda vägen till ett AI STYLE-svar som inte kan avfärdas som stale crawl (`lynx-score` slutsats G). **Kostar en skärmdump. Kan inte tas i efterhand.** *(Sidan är 🔵 → SCORE-vinst, ej flagg-vinst (§1.4); popupen är därför sidans egentliga mätvärde.)*
...
- [ ] ***AI STYLE-TESTET — beställningen LEVERERAD 8 juli 18:13, men testet är fortfarande OBESVARAT.** Popupen gav AI STYLE 70, exakt som FÖRE — **och samtliga övriga 12 delvärden också** (Overall 74, CQ 85, Base 85, Depth 84, Evidence 76, Language 95, Readability 90, EEAT 60, 85/80/80/85). Tredje bit-identiska replikationen. Language stod stilla trots språkfixar; Evidence trots namngivna metoder. **Slutsats: SCORE-vyn har inte läst V2** — `"Analyzed just now"` daterar renderingen, inte crawlen (`lynx-score` slutsats E, ny mätregel 6). **Att läsa 70→70 som "de-AI flyttar inte AI STYLE" vore attribution på stale crawl (mätregel 4).**
  ...
  **⚠️ Men rhinophyma är fel sida för testet** (`lynx-score` slutsats G): den ligger vid SCORE-taket, så ingenting KAN röra sig, så färskheten kan aldrig bevisas där. **Testet hör hemma på en sida med golv — det ligger som ⛔-förvillkor på `microneedling`-raden i Sidarbete, och begärs när den sidan väljs.** Ingen stående beställning här: popupen är färskvara som bara betyder något direkt före main-push.
...
- [medel] **≈25 FLER `$specialist_url_title`-varianter med samma mallfel, sajtbrett** (kartlagt vid sweepen 10 juli, `grep specialist_url_title`): särskrivning + Title Case i mönstret "X Specialiserade Hudterapeuter" ("Rosacea" ×2, "Ärr", "Pormask", "Pigmentfläcks", "Perioral dermatit", "Seborré", "Blandhy", "acneärr", "mjalleksem", "fet hy", nakna "Specialiserade Hudterapeuter" ×5 m.fl.) + trasig grammatik ("På ärr från finnar Specialiserade…") + typos ("Specialisserade", "specialisterade" på hudforandringar) + inkonsekvent versalisering ("Hudterapeuter Specialiserade på Ytliga blodkärl" m.fl.) — siddata §13.K, samma klass som den svepta raden — skulle kräva ett nytt språk-pass med per-sida-ämneskoll (≈25 filer).
- [medel] **Två boilerplate-bildtitlar med samma felklass:** `image_title: 'Dermapen - Microneedling Verktyg för Hudföryngring'` (**53 filer**) och `'CryoPen - Kryoterapi Verktyg för Hudbehandlingar'` (**11 filer**) — särskrivning + Title Case; siddata §13.K — mekaniskt tvåsträngs-svep om ägaren vill.

*(Senast stängda fynd 10 juli: mellanslaget-före-punkt på rhinophyma — fanns INTE i källan eller live (`näsan</a>.` curl-verifierad mot produktion); LYNX textextraktion infogar mellanslag när inline-länken strippas. Not i `lynx-examples`, logg #45. Dessförinnan 8 juli: §13.O:s styckeräkning, logg #39. Samma dag stängdes bildbuggen på microneedling-kortet — rättad på ägarval, logg #45.)*
