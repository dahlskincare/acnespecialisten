...
> **SCORE-kolumnens datafil** (filkarta i `lynx-data.php`). Innehåller alla SCORE-nedbrytningar (Overall · CONTENT QUALITY [Base/Depth/Evidence/Language/Readability] · E-E-A-T [Experience/Expertise/Authority/Trust] · AI STYLE [Human-like]). Tolkningsmodell: playbook **§1.2 (OMPRÖVAS)** — ⚠️ LYNX verkar ha bytt version/skala: **GAMLA analyser** ("Analyzed >30 days ago") passar multiplikativa formeln Overall ≈ CQ×EEAT×AI/3640 (6/6); **FÄRSKA analyser** passar ≈ medel(CQ-badge, EEAT-badge). Jämför ALDRIG färsk mot gammal (mätregel §11.1). **SCORE 0/blank = pågående omanalys/indexering som sätter sig SJÄLV (ägarbesked 7 jul — "trasig" var överdrift):** acne-rygg 0→69, sebo 0→74, micro-nollor→komplett, alla utan omkörning → be inte om omkörning, vänta in nästa dump. (SCORE-kolumnens officiella tooltip saknas ännu — beställd.)
...
## 🗄️ HISTORIK — SUPERSEDED MODELL-LAGER (flyttade hit ur `lynx-models` §1.2 den 8 juli, §9.0 steg 5)
> **Dessa lager är ERSATTA och får inte styra beslut.** Gällande operativ slutsats: `lynx-models` §1.2 (Overall = medel(CQ,EEAT) på färska analyser; EEAT låst 60; taket ~75–77; jaga inte SCORE med copy). Bevarade här därför att (a) den multiplikativa modellen fortfarande beskriver GAMLA analyser (>30 dgr) 7/7 och behövs när en gammal breakdown dyker upp, och (b) marginalkalkylerna är forensik om AI STYLE-hävstången visar sig verklig. **KIND: HISTORIK — uppdateras aldrig retroaktivt.**
>
> ⚠️ **Två påståenden här nere är sedan dess FALSIFIERADE och får inte läsas som gällande:** (1) *"SCORE 0 = trasig/tom analys → omkörning krävs"* — ägarbesked 7 juli: **0/blank = pågående omanalys som sätter sig själv; be aldrig om omkörning** (acne-rygg 0→69, sebo 0→74, acnearr 0→74). (2) *"EEAT klampas på hälsosidor / YMYL"* — slutsats B ovan: klampen följer /varumarken/-URL-strukturen, inte ämnet.
